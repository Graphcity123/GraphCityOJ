...
async def _register_default_languages():
    ...
    for lang in defaults:
        await save_language(lang["name"], dict(lang))
...
async def export_data(req: Request):
    require_admin(req)
    users = []
    for u in (await get_users()).values():
        users.append({
            "user_id": u["user_id"],
            "username": u["username"],
    ...
    problems = []
    for p in (await get_problems()).values():
        problems.append({
            "id": p["id"],
            "title": p["title"],
    ...
    submissions = []
    for s in (await get_submissions()).values():
        submissions.append({
...
async def import_data(req: Request, file: UploadFile = File(...)):
    ...
    if "submissions" in data:
        ...
        for s in data["submissions"]:
            ...
            sid = s["submission_id"]
            await save_submission(sid, dict(s))
