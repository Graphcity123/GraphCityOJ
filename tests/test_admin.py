...
async def admin_client(client):
    ...
    resp = await client.post('/api/user/login', json={'username': 'admin', 'password': 'admintestpassword'})
...
class TestImportData:
    ...
    async def test_import_users(self, admin_client):
        ...
        assert resp.status_code == 200
        resp = await admin_client.get('/api/user/list')
    ...
    async def test_import_conflict(self, admin_client):
        await admin_client.post('/api/user/register', json={'username': 'merge_test', 'password': 'old_password'})
        await admin_client.post('/api/user/login', json={'username': 'admin', 'password': 'admintestpassword'})
        resp = await admin_client.get('/api/user/list')
        ...
        resp = await admin_client.get('/api/user/info/' + uid)
